...
class ListsSuite {
  ...
  @Test def `sum of empty`: Unit = {
    Assert.assertEquals(0, sum(List()))
  }

  @Test def `sum of negatives`: Unit = {
    Assert.assertEquals(0, sum(List(-1, -2, 3)))
  }

  @Test def `sum of duplicates`: Unit = {
    Assert.assertEquals(5, sum(List(1, 1, 1, 2)))
  }

  @Test def `tailrec tests`: Unit = {
    assert(sumTR(List(1, 2, 0)) == 3)
    Assert.assertEquals(0, sumTR(List()))
    Assert.assertEquals(0, sumTR(List(-1, -2, 3)))
    Assert.assertEquals(5, sumTR(List(1, 1, 1, 2)))
  }
  ...
  @Test def `max of empty numbers`: Unit = {
    try {
      max(List())
      Assert.fail("No exception has been thrown")
    } catch {
  ...
  @Test def `max of 0`: Unit = {
    Assert.assertEquals(0, max(List(0)))
  }

  @Test def `max of negatives`: Unit = {
    Assert.assertEquals(-1, max(List(-10, -1, -1000)))
  }

  @Test def `max of duplicates`: Unit = {
    Assert.assertEquals(1000, max(List(0, 0, -1, 1000, 10)))
  }
